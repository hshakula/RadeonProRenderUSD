...
bool RprUsdImageCache::InitCacheKey(std::string const& path, CacheKey* key) {
    if (!ArchGetModificationTime(path.c_str(), &key->modificationTime)) return false;

    int64_t sizeInt = ArchGetFileLength(path.c_str());
    if (sizeInt == -1) return false;

    key->size = static_cast<size_t>(sizeInt);
    key->path = path;
...
RprUsdImageCache::GetImage(
    ...
    CacheKey key;
    CacheEntry::Desc desc;
    if (!InitCacheKey(path, &key) ||
        !InitCacheEntryDesc(data[0].textureData, wrapType, forceLinearSpace, &desc)) {
        return nullptr;
    }
    ...
    auto coreImage = RprUsdCoreImage::Create(m_context, data);
    if (!coreImage) {
        return nullptr;
    }
...
void RprUsdImageCache::PopCacheEntry(
    ...
    auto it = m_cache.find(key);
    if (it == m_cache.end()) {
        TF_CODING_ERROR("Failed to release cache entry: no entry with such key - path=%s, size=%zu, modificationTime=%g",
            key.path.c_str(), key.size, key.modificationTime);
        return;
    }
...
size_t RprUsdImageCache::CacheKey::Hash::operator()(CacheKey const& key) const {
    return GetHash(key.modificationTime) ^ GetHash(key.size) ^ GetHash(key.path);
}
