...
std::string HdRprIpcDelegate::ProcessCommand(std::string const& command) {
    if (_tokens->getStage == command) {
        m_renderParam->RequestFullStageResync();
        ...
        if (!m_renderParam->IsFullStageResyncRequested()) {
            // XXX: Do we need this command at all?
        }
    } else {
        return "unknown";
    }
