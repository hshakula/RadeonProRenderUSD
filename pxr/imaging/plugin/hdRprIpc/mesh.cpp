...
void HdRprMesh::Sync(HdSceneDelegate* sceneDelegate,
    ...
    if (!m_stage) {
        ...
        m_mesh = UsdGeomMesh::Define(m_stage, id);
        m_stage->SetDefaultPrim(m_mesh.GetPrim());

        rprRenderParam->activeLayers.emplace(id, this);
    }
    ...
    if (updateLayer) {
        ...
        rprRenderParam->ipcServer->GetSender(&m_sender);
        m_sender->SendLayer(id, GetEncodedStage());
    }
...
void HdRprMesh::Finalize(HdRenderParam* renderParam) {
    if (m_stage) {
        ...
        rprRenderParam->ipcServer->GetSender(&m_sender);
        m_sender->RemoveLayer(GetId());

        rprRenderParam->activeLayers.erase(GetId());
    }
